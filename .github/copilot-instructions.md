...
# ioBroker.habpanel - GitHub Copilot Instructions
...
### HABpanel Adapter Context

**Primary Purpose**: This adapter provides a lightweight, web-based dashboard interface for ioBroker, based on OpenHAB's HABpanel. It allows users to create custom visualization dashboards with drag-and-drop functionality.

**Key Features**:
- Embedded dashboard designer for building interfaces directly on target devices
- Widget-based dashboard construction with drag-and-drop support
- Mobile-responsive interface using jQuery Mobile
- Real-time data visualization and control
- Server-side panel configuration storage
- Web extension integration with ioBroker's web adapter

**Dependencies**:
- **Web Adapter** (>=3.0.12): Required for serving the web interface
- **Admin Adapter** (>=5.1.13): Required for administrative functions
- **js-controller** (>=5.0.0): Core ioBroker functionality

**Architecture**:
- Mode: Extension adapter (extends web adapter functionality)
- Type: Visualization adapter
- Connection: Local (no external service dependencies)
- Data Flow: Push-based real-time updates
...
### HABpanel-Specific Coding Patterns

**Web Extension Patterns**:
- Follow ioBroker web extension conventions for API integration
- Use `webExtension` property in io-package.json to define entry point (`lib/habpanelapi.js`)
- Implement proper authentication and session handling for web interface

**Dashboard Configuration Handling**:
- Panel configurations are stored server-side using ioBroker's state system
- Use `preserveSettings` configuration ("habpanel") for maintaining user dashboards
- Implement proper JSON validation for dashboard configurations

**Widget Architecture**:
- Widget definitions should be modular and reusable
- Support both boolean and value-based state updates
- Implement proper error handling for missing or invalid state objects
- Use consistent naming conventions for widget parameters

**Frontend-Backend Communication**:
- Leverage ioBroker's websocket communication for real-time updates
- Implement proper error handling for connection issues
- Use push-based data updates for optimal performance
- Support graceful degradation when web adapter is unavailable